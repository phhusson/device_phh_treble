...
if mount -o remount,rw /system;then
	resize2fs $(grep '[^ ]* on /system type ' /proc/mounts | cut -d ' ' -f 1) || true
...
if [ "$vndk" == 27 -o "$vndk" == 26 ];then
	mount -o bind /system/phh/libnfc-nci-oreo.conf /system/etc/libnfc-nci.conf #TODO: Don't do this if we are on a supported Huawei
fi
